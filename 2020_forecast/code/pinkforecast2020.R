...
predict(model.m2, newdata, interval="prediction", level = 0.80) #prediction interval
...
lm_out_seak %>% 
  ...
  labs(y = "Residuals (millions)", x =  "CPUE") +
...
lm_out_seak %>% 
  ...
  labs(y = "Residuals (millions)", x =  "ISTI_MJJ") +
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                     panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+
...
lm_out_seak %>% 
  ...
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                     panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) +
  labs(y = "Residuals (millions)", x =  "Fitted values") +
...
m2 %>%
  ...
  geom_segment(aes(x = 2020, y = 20.1, yend = 0, xend = 2020), size=1, colour="black", lty=1) +
...
m2 %>%
  ...
  geom_segment(aes(x = 1.202607, y = 20.1, yend = 0, xend = 1.202607), size=1, colour="black", lty=1)+  
  geom_text(aes(x = 0, y = 100, label="b)"),family="Times New Roman", colour="black", size=5)-> plot2
cowplot::plot_grid(plot1, plot2,  align = "vh", nrow = 1, ncol=2)
