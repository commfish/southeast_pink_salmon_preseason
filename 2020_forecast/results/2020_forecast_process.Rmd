...
where $y$ is ln(harvest), $\beta_1$ is the coefficient for ln(CPUE+1), $\beta_2$ is the coefficient for the natural log of the environmental covariate water temperature (either May through July average temperature 'ISTI_MJJ_log' or May through August average temperature 'ISTI_log' in the upper 20 m at eight stations in Icy Strait), $\beta_3$ is the interaction term, and $\epsilon$ represents the normally distributed error term. Leave-one-out cross validation (hindcast) and model performance metrics such as Mean and Median Absolute Percentage Error (MAPE, MEAPE), and mean absolute scaled error (MASE)(Hyndman and Kohler 2006) were then used to evaluate forecast accuracy of alternative models. Statistical analyses were performed with the R Project for Statistical computing version 3.6.0 (R Core Team 2019). 
...
The characteristics of a unbiased residual vs. fitted plot and what they suggest about the appropriateness of the simple linear regression model:
...
The Bonferroni outlier test for model 2 suggested that there were no outliers, althouth observation 16 was the most extreme (juvenile year 2012) based on standardized residuals (Table 3). The CPUE term was significant in the lack-of-fit curvature test ($P$<0.05), suggesting some lack of fit for this term.
