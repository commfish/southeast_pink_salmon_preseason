...
tbls_seak$model_summary_table6_m11 %>% 
   knitr::kable(format = 'pandoc', caption = 'Detailed output for model m11. Juvenile years 1998, 2005, 2012, and 2019, and 2020 (years 1999, 2006, 2013, 2020, and 2021) show the largest standardized residual (Std. residuals). Fitted values are bias-corrected.')
```
...
Figure A4: A. SEAK pink salmon harvest (millions) by year with the model fit (line) based upon the base model (model m2). The predicted 2023 forecast is symbolized as a grey circle with an 80% prediction interval (12.9 to 28.5 million fish). B. SEAK pink salmon harvest (millions) against the fitted values from model m2 by year. The dotted line is a one to one reference line. 
...
Figure B1: A. SEAK pink salmon harvest (millions) by year with forecasts for models m1, m2, and m11 for the last ten years.
