...
variables %>% 
  mutate (SEAKCatch_log = log(SEAKCatch),
          ISTI_log = log(ISTI)) %>% # log catcah variable
...
eda.norm(log_data$ISTI_log)# data is normal if the p-value is above 0.05.
...
model.formulas<-c(SEAKCatch_log ~ CPUE,
                 ...
                 SEAKCatch_log ~ CPUE+ISTI_log) # temp. data logged
...
lm(SEAKCatch_log ~ CPUE*ISTI, data = log_data_subset) -> m4
...
augment(best.model) %>% 
  mutate(year = 1997:year.data.one, 
         ...
         sigma = .sigma,
         fit = exp(.fitted) * exp(0.5*sigma*sigma))  -> best.model
