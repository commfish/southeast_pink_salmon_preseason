...
author: "Sara Miller, Rich Brenner, and Jim Murphy"
date: "October 2020"
output:
...
fs::dir_ls(here::here('2021_forecast/results'), regexp = "\\.csv$") %>% 
...
# Executive Summary
Forecasts were developed using an approach described in Murphy et al. 2019. A multiple regression model was developed using monthly peak juvenile CPUE (standardized catch based on 20 minute trawl set) for the June and July surveys and associated environmental parameters. The model used was:
  
$$E(y) = \alpha + \beta_1{X_1} + \beta_2{X_2} + \beta_3{X_1X_2} + \epsilon$$,

where $y$ is ln(harvest), $\beta_1$ is the coefficient for ln(CPUE+1), $\beta_2$ is the coefficient for the environmental covariate water temperature (i.e., May through July average temperature 'ISTI' in the upper 20 m at eight stations in Icy Strait or the natural logarithm of 'ISTI'), $\beta_3$ is the interaction term, and $\epsilon$ represents the normal error term that is lognormal. 
...
Based on model m2, the SEAK pink salmon harvest in 2021 is predicted to be in the strong range with a point estimate of 31.4 million fish (80% prediction interval: 21.0 to 47.0 million fish).
...
where ${X_1}$ was ln(CPUE+1) and ${X_2}$ was either the average temperature in Icy Strait in May, June, and July or the natural logarithm pf the average temperature in Icy Strait in May, June, and July, and $\beta_3$ is the interaction term between CPUE and one of the temperature indices. 

The regression coefficients CPUE and temperature (ISTI_log, ISTI) were significant in the first three models (m1, m2, m3). The interaction terms were not significant in either model m4 or m5 (Table 1). Therefore, only the first three models will be considered further.
...
knitr::include_graphics(here("2021_forecast/results/figs/cpue_temp.png"))
```
...
```{r fitted, echo=FALSE}
knitr::include_graphics(here("2021_forecast/results/figs/fitted.png"))
```
...
knitr::include_graphics(here("2021_forecast/results/figs/predicted.png"))
```
...
knitr::include_graphics(here("2021_forecast/results/figs/influential.png"))
```
...
#Results
The best regression model based on AICc, the MASE metric, significant coefficients in the models, and the argument for restricting temperature to the months when CPUE is sampled was model m2 (i.e. the model containing CPUE and May through July temperature). Diagnostics indicated some observations had high leverage values, but none of the observations affected model fitting and overall the model showed some lack of fit. A few of the data points were above the cut-off value for the Cook's distance. Based on the Bonferroni outlier test, none of the data points had a studentized residual with a significant Bonferroni $P$-value suggesting that none of the data points impacted the model fitting. The conditional mean function in the residual plots should be constant across the plot in a "correct"" model. Based on the results of the curvature test, and the slightly curved fitted lines in the residual versus fitted plot, the fitted plot shows some lack of fit of the model. The adjusted $R^2$ value was 0.81 indicating overall a good model fit.

#Conclusion 
The SEAK pink salmon harvest in 2021 is predicted to be in the strong range with a point estimate of 31.4 million fish (80% prediction interval: 21.0 to 47.0 million fish).

```{r pred, echo=FALSE}
knitr::include_graphics(here("2021_forecast/results/figs/catch_plot_pred.png"))
```

Figure 5: SEAK harvest (millions) a) by year and b) by the fitted values from model m2. The line in figure b is a one to one line. The predicted 2021 forecast is symbolized as a circle with an 80% prediction interval (21.0 to 47.0 million fish) in figure a. 
