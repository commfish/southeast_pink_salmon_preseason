...
where $y$ is ln(harvest), $\beta_1$ is the coefficient for ln(CPUE+1), $\beta_2$ is the coefficient for environmental covariate water temperature (i.e., May through July average temperature 'ISTI' in the upper 20 m at eight stations in Icy Strait or the natural logarithm of 'ISTI'), $\beta_3$ is the interaction term, and $\epsilon$ represents the normal error term that is lognormal. 
...
Based on model 2, the SEAK pink salmon harvest in 2021 is predicted to be in the strong range with a point estimate of 31.4 million fish (80% prediction interval: 21.0 to 47.0 million fish).

#Analysis
Five hierarchical models were investigated. The full model was:
$$E(y) = \alpha + \beta_1{X_1} + \beta_2{X_2} + \beta_3{X_1X_2}$$

where ${X_1}$ was ln(CPUE+1) and ${X_2}$ was either the natural log of the average temperature in Icy Strait in May, June, and July or the average temperature in Icy Strait in May, June, and July, and $\beta_3$ is the interaction term between CPUE and one of the temperature indices. 

The regression coefficients CPUE and temperature (ISTI_log, ISTI) are significant in the first three models (m1, m2, m3). The interaction terms are not significant in either model 4 or 5 (m4 or m5; Table 1). Therefore, only the first three models will be considered further.
...
Figure 5: SEAK harvest (millions) a) by year and b) by the fitted values from model 2. The line in figure b is a one to one line. The predicted 2021 forecast is symbolized as a star with an 80% prediction interval (21.0 to 47.0 million fish) in figure a. 
