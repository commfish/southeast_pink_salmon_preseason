...
where $y$ is ln(harvest), $\beta_1$ is the coefficient for ln(CPUE+1), $\beta_2$ is the coefficient for environmental covariate water temperature (i.e., May through July average temperature 'ISTI'in the upper 20 m at eight stations in Icy Strait), $\beta_3$ is the interaction term, and $\epsilon$ represents the normal error term that is lognormal. This year there was no survey in August. Therefore, the variable ISTI could not be calculated and was not included as a possible variable in the model. 
