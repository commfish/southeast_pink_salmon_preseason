...
Figure 1: A. SEAK pink salmon harvest (millions) by year with the model fit (line) based upon the best performing model (model m2a). The predicted 2026 forecast (22.5 million fish) is symbolized as a grey circle with an 80% prediction interval (13.6 to 37.3 million fish). B. SEAK pink salmon harvest (millions) against the fitted values from model m2a by year. The dotted line is a one to one reference line.

```{r best model m7, echo=FALSE}
knitr::include_graphics(here(results.directory, "/model_figs/catch_plot_pred_m7.png"))
```

Figure 2: A. SEAK pink salmon harvest (millions) by year with the model fit (line) based upon the best performing model (model m2a). The predicted 2026 forecast (18.0 million fish) is symbolized as a grey circle with an 80% prediction interval (9.6 to 33.7 million fish). B. SEAK pink salmon harvest (millions) against the fitted values from model m7 by year. The dotted line is a one to one reference line.
