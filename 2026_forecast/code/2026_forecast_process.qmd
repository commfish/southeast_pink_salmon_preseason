...
Figure 1: A. SEAK pink salmon harvest (millions) by year with the model fit (line) based upon the best performing model (model m13a). The predicted 2026 forecast (19.3 million fish) is symbolized as a grey circle with an 80% prediction interval (12.6 to 29.7 million fish). B. SEAK pink salmon harvest (millions) against the fitted values from model m13a by year. The dotted line is a one to one reference line.
